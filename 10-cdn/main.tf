resource "aws_cloudfront_distribution" "devopsprocloud" {
  enabled = true
  ...
  records = [
    {
      ...
      type = "A"
      alias = {
        name    = aws_cloudfront_distribution.devopsprocloud.domain_name
        zone_id = aws_cloudfront_distribution.devopsprocloud.hosted_zone_id
      }
    }
  ]
}
