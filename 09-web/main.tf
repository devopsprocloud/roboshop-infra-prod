...
# Provisioning the Web AMI with Web data using ansible and bootstrap,sh
#------------------------------------------------------------------------------------
resource "null_resource" "web" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "sudo sh /tmp/bootstrap.sh web dev" # We are parsing the variables as $1 (mongodb) and $2 (dev) to bootstrap script
    ]
  }
}

# Stopping the Web AMI Instnace
#------------------------------------------------------
resource "aws_ec2_instance_state" "stop_web_instance" {
      instance_id = module.web.id
      state       = "stopped"
      depends_on = [ null_resource.web ]
}

# Taking the AMI from Web AMI Instance
#------------------------------------------------------
resource "aws_ami_from_instance" "web" {
      name        = "${local.name}-${var.tags.Component}-${local.current_time}"
      source_instance_id = module.web.id
      depends_on = [ aws_ec2_instance_state.stop_web_instance ] # I've made changes here (added this)
  tags = {
    Name = "${var.tags.Component}-ami"
  }
}

# Deleting the Web AMI Instance
#------------------------------------------------------
resource "null_resource" "web_delete" {
  ...
  triggers = {
    ...
    instance_id = module.web.id # It will trgger when catalogue instance id change 
  }
...
resource "aws_lb_listener_rule" "web" {
  ...
  condition {
    host_header {
      values = ["${var.tags.Component}-${var.environment}.${var.zone_name}"] # web-dev.devopsprocloud.in
    }
  }
